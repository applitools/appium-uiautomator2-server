...
public class GetElementAttribute extends SafeRequestHandler {
    ...
    private static int getScrollableOffsetByHiddenView(UiObject2 helperView) {
        Logger.debug("Figuring out scrollableOffset via EyesAppiumHelper view");

        // here we get the hidden EyesAppiumHelper view from main layout that call some logic to calculate scrollable offset
        // for ListView, GridView and RecyclerView
        try {
            helperView.click();
            helperView.wait(Until.textContains("offset"), 2000);
            String textFromView = helperView.getText();

            int totalItemsHeight = Integer.valueOf(textFromView.replace("offset=", ""));
